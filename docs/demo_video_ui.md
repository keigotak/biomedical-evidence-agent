...
| 1 | 0:07–0:29 | **Animated concept explainer** — a claim → a "smooth answer" that dims and reveals the disagreement / overclaim / unchecked-citation it buried → *Summarize ✗ → AUDIT* with a supporting/conflicting/verbatim split and a CONTESTED pill | Every day, researchers read the biomedical literature — and so do the models we build on top of it. The danger isn't failing to find an answer. It's getting a confident answer that quietly buries the disagreement underneath. BioClaim Auditor takes a different stance. It doesn't summarize a claim. It audits it. |
...
2. `web/_capture.mjs` / `_recap.mjs` (Playwright) — title/close cards and the
   four interactive clips (audit, Evidence Map, overclaim, resolution loop),
   each recorded to its beat length. The concept intro (beat 1) is a
   self-contained animated HTML page (`concept.html`) recorded the same way.
   Captions are burned from `cap_en.srt` / `cap_ja.srt` (generated by
   `gen_srt.py`) with ffmpeg's `subtitles` filter.
